...
void Overlay::CreateOverlay(const char* window_name)
{
	...
	wca.style         = CS_CLASSDC;
	wca.lpfnWndProc   = window_procedure;
	wca.hInstance     = GetModuleHandleA(nullptr);
	wca.lpszClassName = "carlgwastaken";

	RegisterClassExA(&wca);

	// guardamos en el miembro wc (que puede ser WNDCLASSEX Unicode) sólo el hInstance
	wc = *reinterpret_cast<WNDCLASSEX*>(&wca); // trick: mismo layout binario
...
void Overlay::DestroyOverlay()
{
	...
	UnregisterClassA("carlgwastaken", GetModuleHandleA(nullptr));
}
