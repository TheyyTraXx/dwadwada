...
// Source: https://github.com/ocornut/imgui (MIT License)
...
struct VERTEX_CONSTANT_BUFFER_DX11 { float mvp[4][4]; };
...
static void ImGui_ImplDX11_SetupRenderState(const ImDrawData* draw_data, ID3D11DeviceContext* device_ctx)
{
    ...
    vp.MinDepth = 0.0f; vp.MaxDepth = 1.0f; vp.TopLeftX = vp.TopLeftY = 0;
    ...
    if (device_ctx->Map(bd->pVertexConstantBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped_resource) == S_OK)
    {
        ...
        float L = draw_data->DisplayPos.x, R = draw_data->DisplayPos.x + draw_data->DisplaySize.x;
        float T = draw_data->DisplayPos.y, B = draw_data->DisplayPos.y + draw_data->DisplaySize.y;
        float mvp[4][4] = {
...
void ImGui_ImplDX11_RenderDrawData(ImDrawData* draw_data)
{
    ...
    if (!bd->pVB || bd->VertexBufferSize < draw_data->TotalVtxCount) {
        ...
        D3D11_BUFFER_DESC desc = {}; desc.Usage = D3D11_USAGE_DYNAMIC;
    ...
    for (const ImDrawList* draw_list : draw_data->CmdLists) {
        ...
        vtx_dst += draw_list->VtxBuffer.Size; idx_dst += draw_list->IdxBuffer.Size;
    }
    device->Unmap(bd->pVB, 0); device->Unmap(bd->pIB, 0);

    struct BACKUP_DX11_STATE {
        UINT ScissorRectsCount, ViewportsCount;
        D3D11_RECT ScissorRects[D3D11_VIEWPORT_AND_SCISSORRECT_OBJECT_COUNT_PER_PIPELINE];
        D3D11_VIEWPORT Viewports[D3D11_VIEWPORT_AND_SCISSORRECT_OBJECT_COUNT_PER_PIPELINE];
        ID3D11RasterizerState* RS; ID3D11BlendState* BlendState; FLOAT BlendFactor[4]; UINT SampleMask;
        UINT StencilRef; ID3D11DepthStencilState* DepthStencilState;
        ID3D11ShaderResourceView* PSShaderResource; ID3D11SamplerState* PSSampler;
        ID3D11PixelShader* PS; ID3D11VertexShader* VS; ID3D11GeometryShader* GS;
        UINT PSInstancesCount, VSInstancesCount, GSInstancesCount;
        ID3D11ClassInstance *PSInstances[256], *VSInstances[256], *GSInstances[256];
        D3D11_PRIMITIVE_TOPOLOGY PrimitiveTopology;
        ID3D11Buffer *IndexBuffer, *VertexBuffer, *VSConstantBuffer;
        UINT IndexBufferOffset, VertexBufferStride, VertexBufferOffset;
        DXGI_FORMAT IndexBufferFormat; ID3D11InputLayout* InputLayout;
    };
    ...
    int global_idx_offset = 0, global_vtx_offset = 0;
    ImVec2 clip_off = draw_data->DisplayPos, clip_scale = draw_data->FramebufferScale;
    for (const ImDrawList* draw_list : draw_data->CmdLists) {
...
void ImGui_ImplDX11_UpdateTexture(ImTextureData* tex)
{
    ...
    if (tex->Status == ImTextureStatus_WantCreate) {
        ...
        desc.Width = (UINT)tex->Width; desc.Height = (UINT)tex->Height;
        ...
        subResource.pSysMem = tex->GetPixels(); subResource.SysMemPitch = desc.Width * 4; subResource.SysMemSlicePitch = 0;
        ...
        srvDesc.Texture2D.MipLevels = desc.MipLevels; srvDesc.Texture2D.MostDetailedMip = 0;
        ...
        tex->SetStatus(ImTextureStatus_OK); tex->BackendUserData = backend_tex;
    } else if (tex->Status == ImTextureStatus_WantUpdates) {
        ImGui_ImplDX11_Texture* backend_tex = (ImGui_ImplDX11_Texture*)tex->BackendUserData;
        for (ImTextureRect& r : tex->Updates) {
            D3D11_BOX box = { (UINT)r.x, (UINT)r.y, 0u, (UINT)(r.x + r.w), (UINT)(r.y + r.h), 1u };
...
bool ImGui_ImplDX11_CreateDeviceObjects()
{
    ...
    // Vertex shader
    {
        ...
        ID3DBlob* vsBlob;
        if (FAILED(D3DCompile(vertexShader, strlen(vertexShader), nullptr, nullptr, nullptr, "main", "vs_4_0", 0, 0, &vsBlob, nullptr))) return false;
        if (bd->pd3dDevice->CreateVertexShader(vsBlob->GetBufferPointer(), vsBlob->GetBufferSize(), nullptr, &bd->pVertexShader) != S_OK) { vsBlob->Release(); return false; }
        D3D11_INPUT_ELEMENT_DESC local_layout[] = {
            { "POSITION", 0, DXGI_FORMAT_R32G32_FLOAT,   0, (UINT)offsetof(ImDrawVert, pos), D3D11_INPUT_PER_VERTEX_DATA, 0 },
            { "TEXCOORD", 0, DXGI_FORMAT_R32G32_FLOAT,   0, (UINT)offsetof(ImDrawVert, uv),  D3D11_INPUT_PER_VERTEX_DATA, 0 },
            { "COLOR",    0, DXGI_FORMAT_R8G8B8A8_UNORM, 0, (UINT)offsetof(ImDrawVert, col), D3D11_INPUT_PER_VERTEX_DATA, 0 },
        };
        if (bd->pd3dDevice->CreateInputLayout(local_layout, 3, vsBlob->GetBufferPointer(), vsBlob->GetBufferSize(), &bd->pInputLayout) != S_OK) { vsBlob->Release(); return false; }
        vsBlob->Release();
        D3D11_BUFFER_DESC desc = {}; desc.ByteWidth = sizeof(VERTEX_CONSTANT_BUFFER_DX11);
    ...
    // Pixel shader
    {
        ...
        ID3DBlob* psBlob;
        if (FAILED(D3DCompile(pixelShader, strlen(pixelShader), nullptr, nullptr, nullptr, "main", "ps_4_0", 0, 0, &psBlob, nullptr))) return false;
        if (bd->pd3dDevice->CreatePixelShader(psBlob->GetBufferPointer(), psBlob->GetBufferSize(), nullptr, &bd->pPixelShader) != S_OK) { psBlob->Release(); return false; }
        psBlob->Release();
    }

    // Blend state
    {
        D3D11_BLEND_DESC desc; ZeroMemory(&desc, sizeof(desc));
        desc.RenderTarget[0].BlendEnable = true;
        desc.RenderTarget[0].SrcBlend = D3D11_BLEND_SRC_ALPHA; desc.RenderTarget[0].DestBlend = D3D11_BLEND_INV_SRC_ALPHA; desc.RenderTarget[0].BlendOp = D3D11_BLEND_OP_ADD;
        desc.RenderTarget[0].SrcBlendAlpha = D3D11_BLEND_ONE; desc.RenderTarget[0].DestBlendAlpha = D3D11_BLEND_INV_SRC_ALPHA; desc.RenderTarget[0].BlendOpAlpha = D3D11_BLEND_OP_ADD;
    ...
    // Rasterizer state
    {
        D3D11_RASTERIZER_DESC desc; ZeroMemory(&desc, sizeof(desc));
        desc.FillMode = D3D11_FILL_SOLID; desc.CullMode = D3D11_CULL_NONE; desc.ScissorEnable = true; desc.DepthClipEnable = true;
        bd->pd3dDevice->CreateRasterizerState(&desc, &bd->pRasterizerState);
    }

    // Depth stencil
    {
        D3D11_DEPTH_STENCIL_DESC desc; ZeroMemory(&desc, sizeof(desc));
        desc.DepthEnable = false; desc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ALL; desc.DepthFunc = D3D11_COMPARISON_ALWAYS;
        desc.FrontFace.StencilFailOp = desc.FrontFace.StencilDepthFailOp = desc.FrontFace.StencilPassOp = D3D11_STENCIL_OP_KEEP;
        desc.FrontFace.StencilFunc = D3D11_COMPARISON_ALWAYS; desc.BackFace = desc.FrontFace;
        bd->pd3dDevice->CreateDepthStencilState(&desc, &bd->pDepthStencilState);
    }

    // Samplers
    {
        D3D11_SAMPLER_DESC desc; ZeroMemory(&desc, sizeof(desc));
        desc.Filter = D3D11_FILTER_MIN_MAG_MIP_LINEAR; desc.AddressU = desc.AddressV = desc.AddressW = D3D11_TEXTURE_ADDRESS_CLAMP;
        desc.ComparisonFunc = D3D11_COMPARISON_ALWAYS; desc.MaxLOD = 0.f;
...
bool ImGui_ImplDX11_Init(ID3D11Device* device, ID3D11DeviceContext* device_context)
{
    ...
    IDXGIDevice* pDXGIDevice = nullptr; IDXGIAdapter* pDXGIAdapter = nullptr; IDXGIFactory* pFactory = nullptr;
    if (device->QueryInterface(IID_PPV_ARGS(&pDXGIDevice)) == S_OK)
        if (pDXGIDevice->GetParent(IID_PPV_ARGS(&pDXGIAdapter)) == S_OK)
            if (pDXGIAdapter->GetParent(IID_PPV_ARGS(&pFactory)) == S_OK) {
                bd->pd3dDevice = device; bd->pd3dDeviceContext = device_context; bd->pFactory = pFactory;
            }
    if (pDXGIDevice) pDXGIDevice->Release();
    if (pDXGIAdapter) pDXGIAdapter->Release();
    bd->pd3dDevice->AddRef(); bd->pd3dDeviceContext->AddRef();
    return true;
}
...
void ImGui_ImplDX11_Shutdown()
{
    ...
    io.BackendRendererName = nullptr; io.BackendRendererUserData = nullptr;
